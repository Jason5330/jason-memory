#!/usr/bin/env python3
"""
jason_doctor — consistency + protocol check for an Jason memory store.

    python tools/jason_doctor.py <MEMORY_ROOT>              # structure + schema
...
def _frontmatter(text):
    # Validate + parse Jason's restricted `key: value` frontmatter between
...
def main(argv):
    ...
    if not os.path.isfile(idx_path):
        print(f"jason-doctor: no index at {idx_path}")
        return 1
    ...
    for base, p in sorted(notes.items()):
        ...
        for w in re.findall(r"\[\[([^\]]+)\]\]", text):
            ...
            if "/" in target or "\\" in target:
                info.append(f"[[{w}]] in {base} isn't a bare slug (jason links are flat "
    ...
    if issues:
        ...
        print(f"jason-doctor: {len(issues)} issue(s) — {summary}")
        for b in order:
            if b in buckets:
    ...
    print(f"jason-doctor: clean — index {nlines} lines / {_kb(nbytes)}, "
