#!/usr/bin/env python3
"""
Jason index-size guard — a Claude Code PreToolUse hook.
...
jason_check.py) instead of passing silently. It still does not deny — a
...
form so the path is passed literally — no shell, so no quoting / backslash pitfalls:
  {"type": "command", "command": "python3",
   "args": ["/ABSOLUTE/PATH/TO/jason/hooks/jason_index_guard.py"]}
...
Config via environment variables (all optional):
  JASON_HARD          hard line ceiling,  default 200
  JASON_WARN          soft line warning,  default 150
  JASON_HARD_BYTES    hard byte ceiling,  default 25600  (25 KB)
  JASON_WARN_BYTES    soft byte warning,  default 20480  (20 KB)
  JASON_INDEX_NAME    index filename to guard, default "MEMORY.md"
  JASON_INDEX_PATH    absolute path of the one index to guard (overrides name
...
def main():
    ...
    index_name = os.environ.get("JASON_INDEX_NAME", "MEMORY.md")
    index_path = os.environ.get("JASON_INDEX_PATH", "")
    ...
    if worsens_cap:
        ...
        _emit(
            "deny",
            reason=(
                f"Jason: this edit would GROW the memory index to {size}, past the "
                ...
                f"(If this file is NOT your memory index, set the JASON_INDEX_PATH env "
        ...
        _emit(
            context=(
                f"Jason: index will be {size}, still over the load window "
        ...
        _emit(
            context=(
                f"Jason: index will be {size} — over "
