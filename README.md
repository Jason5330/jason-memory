...
兩個預設立場（都可以改，`CLAUDE.md` 裡對應的段落本來就是給你直接修改用的）：

- **只限專案層級**：記憶庫只存在專案內的 `<MEMORY_ROOT>/`，不用任何專案外部、
  跟著機器走的全域記憶路徑——換一台機器、換人接手這個專案，記憶庫都跟著 repo 一起走
- **內容預設繁體中文**：筆記的 `description`、內文、`Why:` / `How to apply:` 預設都用
  繁體中文寫；固定的格式代號（`type` 的值、frontmatter 欄位名）維持英文
