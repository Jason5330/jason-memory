...
完整協議在框架的 `SKILL.md`。**使用前，先把下面所有的 `<MEMORY_ROOT>`
換成記憶庫的實際路徑**（例如 `.jason-memory`），並把 `<FRAMEWORK_ROOT>`
換成框架原始碼所在的路徑（例如 `jason-memory` 或絕對路徑）。
...
預設記憶庫位於這個專案內的 `<MEMORY_ROOT>/`；不要因為 host 另有原生記憶
...
使用者明確確認的持久工作偏好（例如回覆語言）也屬於 feedback，不必刻意製造
...
- **在準備做一次刻意的 compact、清空 context、或開新對話之前**，做一次同步：
  ...
  歸檔過時或已完成的暫時性狀態；跑 `<FRAMEWORK_ROOT>/tools/jason_check.py` 跟
  `<FRAMEWORK_ROOT>/tools/jason_doctor.py`；然後確認一個從零開始的 Agent，光靠 repo 加記憶庫
  就能接續工作。
- **寫入/同步完成後**，回報新增（`added`）、更新（`updated`）、歸檔
  （`archived`）、跳過（`skipped`，附原因；歸檔裡若有刪除要特別標明），
  以及索引的行數/位元組數跟檢查結果。健檢不會自動做語意上的同步整理，
...
完整協議與設計理由：`<FRAMEWORK_ROOT>/SKILL.md`。
