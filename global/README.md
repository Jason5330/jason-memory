...
**永久移除：** 關閉 AI 工作階段後，雙擊 `UNINSTALL.bat`，移除框架、全局庫
（含其中所有專案記憶）與 Jason-memory 入口／hooks。保留其他 AI 設定與工作檔案。
刪除不可復原，詳細範圍與預覽方式見 [清除說明](docs/uninstall.md)。
...
新版修正全局 hook 誤擋框架說明檔，簡單偏好以一次 `remember` 呼叫完成保存、
健檢與收據。完整 hook 快照可直接重用；不再每次讀多份說明或額外重跑健檢。
...
每輪第一則可見回覆前，AI 使用 hook 最新完整快照；無 hook 或截斷才執行 `context` 刷新全局與目前專案索引，再讀取
...
本版會一併更新保存證據檢查：成功宣告需要本輪工具收據與實際筆記吻合；
已存在的偏好只驗證、不重寫。沒有證據時要求補寫或更正，重試失敗明確警告。
`.hook-state`、`.receipts`、`.writer.lock` 都是中繼資料，不代表已保存使用者偏好。
